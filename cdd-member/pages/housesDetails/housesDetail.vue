<template>
  <view class="container">
    <view class="contentTop">
      <view class="topTitle">
        <view class="houseName">{{houseDetail.title}}</view>
        <view class="houseStatus" v-if="houseDetail.saleStatus">{{houseDetail.saleStatus}}</view>
      </view>
      <view class="houseLable" v-for="(val,index) in labelsArr" :key='index' v-if="val?val:''">{{val}}</view>
    </view>
    <view class="carousel">
    ...
    <view class="housesDescribe">
      <view class="left">
        <view class="describeLeft1">
          <view class="totalTitle">总面积</view>
          <view class="totalContent" v-if="houseDetail.area">{{houseDetail.area}}㎡</view>
          <view class="totalContent" v-else>-</view>
        </view>
        <view class="describeLeft2">
          <view class="totalTitle">类型：</view>
          <view class="totalContent" v-if="houseDetail.houseUseType">{{houseDetail.houseUseType}}</view>
        </view>
        <view class="describeLeft2" v-if="houseDetail.electricity">
          <view class="totalTitle">配电：</view>
          <view class="totalContent" v-if="houseDetail.electricity">{{houseDetail.electricity}}</view>
          <view class="totalContent" v-else>-</view>
        </view>
		
		<view class="describeLeft2" v-if="houseDetail.floor">
		  <view class="totalTitle">楼层：</view>
		  <view class="totalContent" v-if="houseDetail.floor">{{houseDetail.floor}}</view>
		  <view class="totalContent" v-else>-</view>
		</view>
		...
		<view class="address">
		  ...
		  <view class="addressText">
			<text v-if="houseDetail.city">{{houseDetail.city}}</text>
			<text v-if="houseDetail.county">{{houseDetail.county}}</text>
			<text v-if="houseDetail.town">{{houseDetail.town}}</text>
			<text v-if="houseDetail.street">{{houseDetail.street}}</text>
			<text v-if="houseDetail.staffNumber">{{houseDetail.staffNumber}}</text>
		  </view>
		</view>
	  ...
	  <view class="center">
		  <view class="describeright1">
		    <view class="totalTitle">租金</view>
		    <view class="totalContent" v-if="houseDetail.singlePrice">{{houseDetail.singlePrice}}{{houseDetail.priceType}}</view>
			<view class="totalContent" v-else-if="houseDetail.sellingPrice">{{houseDetail.sellingPrice}}{{houseDetail.priceType}}</view>
		    <view class="totalContent" v-else>-</view>
		  </view>
      ...
      <view class="right">
        <view class="describeright1">
          <view class="totalTitle">可租面积</view>
          <view class="totalContent" v-if="houseDetail.useArea">{{houseDetail.useArea}}㎡</view>
          <view class="totalContent" v-else>-</view>
        </view>
        
		<view class="describeright2">
		  <view class="totalTitle">结构：</view>
		  <view class="totalContent" v-if="houseDetail.houseType">{{houseDetail.houseType}}</view>
		</view>
		<view class="describeright2" v-if="houseDetail.fireControl">
		  <view class="totalTitle">消防：</view>
		  <view class="totalContent" v-if="houseDetail.fireControl">{{houseDetail.fireControl}}</view>
    ...
    <view class="map">
      ...
      <view class="mapMark" @click="navToMap">
        <view class="goNav">{{houseDetail.title}}
          <view class="navTwo">
			<text v-if="houseDetail.city">{{houseDetail.city}}</text>
			<text v-if="houseDetail.county">{{houseDetail.county}}</text>
			<text v-if="houseDetail.town">{{houseDetail.town}}</text>
    ...
    <view class="goods-list1">
      <view class="goods-item" v-for="(val,index) in goodsList" :key='index' @click="navToDetail(val.id)" v-if="index<5">
        ...
        <view class="image-wrapper">
          ...
          <image :src="(val.background.split('|'))[0]" mode="aspectFill"></image>
        </view>
        <view class="listRight">
          <view class="nameBox">
          			<view class="state" v-bind:class="{'chip':val.houseUseType=='出租','shopping':val.houseUseType=='出售','out':val.houseUseType=='求租','out':val.houseUseType=='求购' }" v-if="val.houseUseType">{{val.houseUseType}}</view>
            <view class="houseName">{{val.title}}</view>
            
          </view>
          <view class="houseAddress">
            <text v-if="val.singlePrice">{{val.singlePrice}}</text>
          			<text v-else-if="val.sellingPrice">{{val.sellingPrice}}</text>
          			<text>{{val.priceType}}</text>
            <text class="area">可租面积：{{val.useArea}}㎡</text>
          </view>
          
          <view class="houseAddress">
            
            地址：<text v-if="val.city">{{val.city}}</text>
          			<text v-if="val.county">{{val.county}}</text>
          			<text v-if="val.town">{{val.town}}</text>
          			<text v-if="val.street">{{val.street}}</text>
          			<text v-if="val.staffNumber">{{val.staffNumber}}</text>
          </view>
		  <view class="tagBox">
		    <view class="tagList" :key='index1' v-for="(val1,index1) in labelArr[index]" v-if="val1?val1:''">{{val1}}</view>
		  </view>
        </view>
      </view>
    </view>
    ...
    methods: {
      ...
      getRecommend() {
        this.$api.httpRequest('GET', this.$constant.API_HOME + 'property/property/list/hot',
          ...
          for (var a = 0; a < this.goodsList.length; a++) {
            this.labelArr[a] = [];
            for (var b = 0; b < this.goodsList[a].houseEdge.split('-').length; b++) {
              this.labelArr[a][b] = this.goodsList[a].houseEdge.split('-')[b]
            }
          }
        });
      },
      ...
      getGoodsDetails(id) {
        this.$api.httpRequest(
          'GET',
          this.$constant.API_HOME + 'house/findHouseInfoDetail', {
			  houseId: id
		  }, {
		  ...
		  this.labelsArr = data.houseEdge.split('-');
		  this.goodsList = data.likes;
		  for (var a = 0; a < this.goodsList.length; a++) {
		    this.labelArr[a] = [];
		    for (var b = 0; b < this.goodsList[a].houseEdge.split('-').length; b++) {
		      this.labelArr[a][b] = this.goodsList[a].houseEdge.split('-')[b]
          ...
          this.imgList = this.houseDetail.background.substr(0,this.houseDetail.background.length-1).split('|')
		  console.log(this.imgList)
