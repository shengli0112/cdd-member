...
<!--      </view>-->
	...
	<view class="navbar">
		<view class="nav-item" @click="tabClick(1),tabCurrentIndex=1" :class="{current: tabCurrentIndex === 1}">厂房</view>
		<view class="nav-item" @click="tabClick(2),tabCurrentIndex=2" :class="{current: tabCurrentIndex === 2}">仓库</view>
		<view class="nav-item" @click="tabClick(3),tabCurrentIndex=3" :class="{current: tabCurrentIndex === 3}">土地</view>
		<view class="nav-item" @click="tabClick(3),tabCurrentIndex=4" :class="{current: tabCurrentIndex === 3}">查看更多>></view>
	</view>
...
<script>
    ...
    export default {
        ...
        onShow(){
            ...
            //     this.getRecommend();
            // 	})
            /* if(this.$store.state.hasLogin){ -- gsl 注销
        ...
        methods   :{
            ...
            getRecommend(houseType){
                this.$api.httpRequest('GET', this.$constant.API_HOME + 'house/findHomeHouseInfoList',
                  {
                      pageNo :this.page,
                      pageSize :'10',
					  houseType : houseType,
                  ...
                  // ,{token:this.$store.state.hasLogin ? this.$store.state.userInfo.token.token : '',}
					...
					console.log(data)
                    this.page == Math.ceil(data.houseCount / 10) ? this.loadingType = 'nomore' : this.loadingType = 'more';
                    data.total == 0 ? this.loadingType = 'nomore' : '';
                    if(this.page == 1){
                        this.goodsList = [];
                        this.goodsList = data.houseList;
                    }else{
                        for(var i = 0; i < data.houseList.length; i++){
                            this.goodsList.push(data.houseList[i])
                        }
                    }
            ...
            async loadData(){
				this.$api.httpRequest('GET',this.$constant.API_HOME + 'slide/findSlideList?position=首页').then((data) => {
					console.log(data);
					let carouselList  = data; 
